...
const source = `const CACHE="aurelius-shell-${revision.replace(/[^a-z0-9]/gi, "")}";const ASSETS=${JSON.stringify([basePath, `${basePath}index.html`, ...staticFiles])};self.addEventListener("install",e=>e.waitUntil(Promise.all([caches.open(CACHE).then(c=>c.addAll(ASSETS)),self.skipWaiting()])));self.addEventListener("activate",e=>e.waitUntil(Promise.all([caches.keys().then(keys=>Promise.all(keys.filter(k=>k.startsWith("aurelius-shell-")&&k!==CACHE).map(k=>caches.delete(k)))),self.clients.claim()])));self.addEventListener("message",e=>{if(e.data?.type==="SKIP_WAITING")self.skipWaiting()});self.addEventListener("fetch",e=>{const u=new URL(e.request.url);if(u.origin!==location.origin||e.request.method!=="GET"||u.pathname.includes("/exports/")||u.pathname.includes("/assets/")&&u.search||e.request.headers.has("range"))return;if(e.request.mode==="navigate"){e.respondWith(caches.match("${basePath}index.html").then(r=>r||fetch(e.request)));return}if(ASSETS.includes(u.pathname))e.respondWith(caches.match(e.request).then(r=>r||fetch(e.request)))});`;
